...
import { ItemListPage as ShowCaseList } from 'pages/items';
...
function DashboardPage() {
  ...
  const itemStat = useSWR<PaginatedInterface<ItemInterface>>(
    () => `/items`,
    () => getItems({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Items',
      result: itemStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`items`} passHref legacyBehavior>
                  <Button
